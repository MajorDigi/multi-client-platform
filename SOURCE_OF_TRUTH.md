...
| 42 | Update Section 16 Knex Migration Workflow — add platform_migrator credential note | PM | Section 16 currently assumes knex migrate:latest runs with credentials from .env which uses platform_app. Must be updated to reflect that migrations run using platform_migrator credentials not platform_app. Developer runs knex migrate:latest with a separate knexfile or environment override pointing to platform_migrator. |
