...
- **RDP terminal output credential warning and safety protocol** — when directing the developer to run server-side commands involving credentials, always include this reminder at the bottom of every instruction block: "Security reminder — before pasting any terminal output from the RDP session back into chat, review it line by line for credential values. PowerShell and MySQL may echo passwords in command history, error messages, or command confirmations. Never paste output that contains a password into any chat message." Additionally — if any agent receives a message from the developer that appears to contain a real credential value — a password, token, key, or secret — the agent must immediately: (1) stop all current work, (2) do not repeat, log, store, or act on the credential value in any way, (3) notify the developer immediately that a potential credential was detected in the message, (4) instruct the developer to return to PM Agent for direction on how to assess and contain the exposure before any further work proceeds. No agent continues working after detecting a potential credential in developer input until PM has assessed and cleared the situation.
