...
7. **Commit reporting standard** — developer always reports commit confirmations to PM in the format: Committed [hash] — [commit message]. The commit message must be human-readable as a standalone sentence describing exactly what changed. PM writes commit messages accordingly. Hash alone is never sufficient as a confirmation.
8. **Decisions Log date discipline** — Claude Code does not know the current date independently. It infers dates from document context which is frequently wrong especially across multi-day sessions. PM must explicitly state today's actual date in every command that adds a Decisions Log entry using the format: date this entry YYYY-MM-DD. PM knows today's date from session context. Claude Code only knows what the command text tells it. Never trust date inference from document context.
9. **Claude Code file system access and .env security** — Claude Code has read access to all files in the project directory including .env files containing live credentials. Two safeguards are required: (1) .env files must always be listed in .gitignore before any code is written — this prevents credentials from reaching GitHub, (2) CLAUDE.md must include an explicit instruction telling Claude Code never to read, display, or echo .env file contents in terminal output — this prevents local credential exposure if output is shared or screenshotted. Both safeguards must be in place before Claude Code is used in any project session involving live credentials.
