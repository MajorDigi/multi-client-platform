...
A second practical note for RDP-based server workflows — terminal output from PowerShell and MySQL on a remote server may echo credential values in command history, error messages, or confirmations. Always review terminal output line by line before pasting it back into any agent chat. If an agent receives input that appears to contain a credential it is instructed to stop immediately, notify the developer, and route to PM for exposure assessment before any further work proceeds. This safety protocol is built into every agent in this project via CLAUDE.md.
